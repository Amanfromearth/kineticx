...
const usedata = create((set) => ({
  ...
  updateText: (index, newText) => set((state) => {
    const newData = state.data.map((item, i) => {
      if (i === index) {
...
const useframe = create((set) => ({
  ...
  updateFrame: (newData) => set({ frame: newData }),
}));

export { useframe };

const useframenum = create((set) => ({
  framenum: -1,

  updateframenum: (newData) => set({ framenum: newData }),
}));
...
const usecolor = create((set) => ({
  ...
  updatecolor: (newData) => set({ color: newData }),
}));
